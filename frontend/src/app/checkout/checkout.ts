...
export class CheckoutComponent implements OnInit {
  ...
  today = new Date().toLocaleDateString('en-PH', { year: 'numeric', month: 'short', day: 'numeric' });

  get farmLabel(): string {
    const f = this.cart.checkoutFarm();
    return f ? `Ordering from ${f}` : 'Ordering all items';
  }

  get subtotal() { return this.cart.checkoutTotal(); }
  ...
  ngOnInit() {
    ...
    if (this.cart.checkoutItems().length === 0) this.router.navigate(['/cart']);
  }
  ...
  placeOrder() {
    ...
    const payload = {
      items: this.cart.checkoutItems().map(i => ({
    ...
    this.orderService.create(payload).subscribe({
      next: r => {
        ...
        // Remove only the checked-out items, leave the rest in cart
        const farm = this.cart.checkoutFarm();
        if (farm) {
          this.cart.items()
            .filter(i => i.farmName === farm)
