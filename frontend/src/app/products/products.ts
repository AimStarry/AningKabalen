...
export class ProductsComponent implements OnInit {
  ...
  allListingsUnfiltered: ApiListing[] = []; // full list, never filtered — used for sidebar counts
  ...
  ngOnInit() {
    this.categoryService.getAll().subscribe({
      next: r => {
        ...
        // Subscribe to queryParams observable so re-navigation to /products?category=x
        // (when already on the products page) also triggers a reload
    ...
    // Load full listing list once — used only for sidebar counts
    this.listingService.getAll({ status: 'available', limit: 1000 }).subscribe({
      next: r => { this.allListingsUnfiltered = r.listings; },
      error: () => {}
    });
  ...
  getCountByCategory(catId: string): number {
    return this.allListingsUnfiltered.filter(l => (l.category_id as any)?._id === catId).length;
  }
